...
print("Loading the LaundryLink campus routing network (might take a few seconds)...")
...
def run_astar(graph, start, target):
    # A* implementation using the Haversine heuristic.
...
def write_log(filename, algo_name, start_time, end_time, total_ms, path_len):
    ...
    with open(filename, 'w') as f:
        f.write(f"--- LaundryLink Delivery Route Log ---\n")
...
# -------------- Execution --------------
print("\n=== LaundryLink: Campus Route Optimizer ===")
print("Welcome to the independent logistics dispatch system.")
...
def get_valid_node(prompt_text, graph):
    while True:
        ...
        if user_input.lower() == 'exit':
            print("  Disconnecting from LaundryLink dispatch system...")
            exit()
            
        if not user_input:
            continue
            
        print(f"  Scanning campus grid for '{user_input}'...")
        
        try:
            coords = ox.geocode(user_input)
            node = ox.distance.nearest_nodes(graph, X=coords[1], Y=coords[0])
            print(f"  ✅ Location secured! Snapped to Node ID: {node}")
            return node 
        except Exception:
...
# Get inputs
start_node = get_valid_node("Enter Laundry Pickup Location: ", G)
print("-" * 40)
end_node = get_valid_node("Enter Laundry Drop-off Location: ", G)

print(f"\n[Dispatch System] Calculating optimal delivery route from Node {start_node} -> Node {end_node}")

# --- Run Dijkstra ---
print("\n[1/2] Running baseline route calculation (Dijkstra)...")
...
print(f"> Dijkstra finished in {time_d_ms:.3f} ms. Delivery route is {len(path_d)} intersections long.")
...
# --- Run A* ---
print("\n[2/2] Running advanced heuristic calculation (A*)...")
...
print(f"> A* finished in {time_a_ms:.3f} ms. Delivery route is {len(path_a)} intersections long.")
...
print("\n✅ Dispatch calculations complete! Check your folder for the delivery logs and HTML route maps ✅")